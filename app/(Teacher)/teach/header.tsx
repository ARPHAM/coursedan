                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notificationsData?.notifications?.length ? (
                      notificationsData.notifications.map((n) => (
                        <div
                          key={n.id}
                          className={`p-3 text-xs border-b last:border-b-0 cursor-pointer ${
                            !n.isRead ? "bg-blue-50" : ""
                          }`}
                        >
          ...
          <div className="relative ml-1" ref={userRef}>
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-3 w-40 bg-white rounded-lg shadow-xl border z-30 py-1">
                ...
                <button
                  onClick={() => {
                    /* Thêm logic Đăng xuất tại đây */ alert("Đã đăng xuất!");
                    router.push("/");
                  }}
                  className="flex items-center gap-2 px-4 py-2 text-sm text-red-500 w-full text-left hover:bg-gray-100"
                >
